...
from mylib.json_helper import MergePolicy, type_safe_json_merge

def test_dict_merge_no_extend():
    target = {'key1': 'value1'}
    source = {'key1': 'updated', 'key2': 'value2'}
    result = type_safe_json_merge(target, source)
    assert result == {'key1': 'updated'}

def test_dict_merge_with_extend_policy():
    target = {'key1': 'value1'}
    source = {'key2': 'value2'}
    result = type_safe_json_merge(target, source, merge_policies={'': MergePolicy.MERGE_EXTEND})
...
def test_dict_merge_with_extend_policy_nested():
    ...
    result = type_safe_json_merge(target, source, merge_policies=merge_policies)
...
def test_list_merge_with_extend():
    target = [1, 2, 3]
    source = [4, 5, 6, 7]
    result = type_safe_json_merge(target, source, default_merge_policy=MergePolicy.MERGE_EXTEND)
...
def test_scalar_conversion():
    ...
    def convert_scalar(s, t):
        return int(s)
    result = type_safe_json_merge(target, source, convert_scalar=convert_scalar)
    assert result == 200
...
def test_overwrite_policy():
    target = {'key1': 'value1'}
    source = {'key1': 'updated'}
    result = type_safe_json_merge(target, source, default_merge_policy=MergePolicy.OVERWRITE)
    assert result == {'key1': 'updated'}
