...
def test_list_items(storage_source_manager):
    
    items = storage_source_manager.list_items()

    assert len(items) == 4
    assert items[0] == StorageSourceManager.Item(name='app_item1', source=StorageSourceManager.Source.APP)
    assert items[1] == StorageSourceManager.Item(name='app_item2', source=StorageSourceManager.Source.APP)
    assert items[2] == StorageSourceManager.Item(name='user_item1', source=StorageSourceManager.Source.USER)
    assert items[3] == StorageSourceManager.Item(name='user_item2', source=StorageSourceManager.Source.USER)
...
def test_list_items_only_user_storage():
    ...
    assert len(items) == 1
    assert items[0] == StorageSourceManager.Item(name='user_item1', source=StorageSourceManager.Source.USER)
